...
import { conditionLabel, conditionColor, conditionBadge, formatDate } from '../../utils/helpers';
...
function exportCSV(rows: Structure[], filenameStem: string) {
  const csv = [
    ['ID','Name','Road','Region','Chainage(km)','Lat','Lng','Span(m)','Spans','Lanes','Width(m)',
    ...
    ...rows.map(s => [
      s.id, `"${s.name}"`, `"${s.road}"`, s.region, s.chainage,
...
const COLUMNS: STColumn<Structure>[] = [
...
/** One type's registry: its own toolbar, table and exports - never merged
 * with the other type, per the platform's standing bridges/culverts rule. */
function TypeRegistry({
  title, subtitle, accent, rows, regions, onSelect, columns,
}: {
  title: string; subtitle: string; accent: string; rows: Structure[]; regions: string[];
  onSelect: (s: Structure) => void; columns: STColumn<Structure>[];
}) {
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [rows, query, condFilter, regionFilter]);

  const fileStem = `Department of National Roads_${title.replace(/\s+/g, '_')}_${new Date().toISOString().slice(0,10)}`;

  return (
    <div className="flex flex-col animate-fade-in" style={{ marginBottom: 28 }}>
      <div style={{background:`${accent}0a`,border:`1px solid ${accent}24`,borderRadius:10,padding:'10px 16px',marginBottom:16,display:'flex',alignItems:'center',gap:10,flexWrap:'wrap'}}>
      ...
      <div className="flex-shrink-0 pb-4 border-b border-slate-700/60">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <SearchableSelect value={regionFilter} onChange={setRegionFilter} style={filterSelectStyle}>
            {regions.map(r => (
          ...
          <span className="text-xs text-slate-500">{filtered.length.toLocaleString()} {title.toLowerCase()}</span>
          <button onClick={() => exportCSV(filtered, fileStem)} className="bms-btn-secondary text-xs py-1.5">
            <Download size={13} /> CSV
          </button>
          <button
            onClick={() => downloadGeoJSON(filtered, `${fileStem}.geojson`)}
            className="bms-btn-secondary text-xs py-1.5"
          >
            <Download size={13} /> GeoJSON
          </button>
          <button
            onClick={() => downloadKML(filtered, `${fileStem}.kml`)}
            className="bms-btn-secondary text-xs py-1.5"
          >
...
export default function StructureRegistry() {
  ...
  const bridges  = useMemo(() => structures.filter(s => s.type === 'bridge'),  [structures]);
  const culverts = useMemo(() => structures.filter(s => s.type === 'culvert'), [structures]);

  const bridgeRegions = useMemo(() => {
    const r = new Set(bridges.map(s => s.region).filter(Boolean));
    return ['all', ...Array.from(r).sort()];
  }, [bridges]);
  const culvertRegions = useMemo(() => {
    const r = new Set(culverts.map(s => s.region).filter(Boolean));
    return ['all', ...Array.from(r).sort()];
  }, [culverts]);

  // Detail-view column shares the row's onClick with the modal opener.
  ...
  return (
    <div className="flex flex-col h-full animate-fade-in overflow-auto p-4">
      <TypeRegistry
        title="Bridges" subtitle="MoWT · Bridge Inventory · NBI"
        accent="#00f5ff" rows={bridges} regions={bridgeRegions}
        onSelect={setSelected} columns={columns}
      />
      <TypeRegistry
        title="Major Culverts" subtitle="MoWT · Culvert Inventory"
        accent="#b967ff" rows={culverts} regions={culvertRegions}
        onSelect={setSelected} columns={columns}
      />
