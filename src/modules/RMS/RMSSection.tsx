...
const TIERS = [
  ...
  },
  {
    ...
    tools: ['GeoJSON network (1,017 links)', 'SQLite traffic_platform.db', 'Bridge & culvert registry', 'Condition survey records', 'Maintenance station records'],
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: 'rgba(203,213,225,0.8)', lineHeight: 1.7 }}>
              ...
              road network assets throughout their life cycle. It covers the full{' '}
              <strong>{OFFICIAL_NETWORK_KM.toLocaleString()} km</strong> national road network managed by
              the Department of National Roads (DNR), incorporating Pavement Management (PMS),
              Bridge Management (BMS), Traffic Information (TIS), Investment Planning (NDPIV),
              Output-based Contracts (OPRC), Life Cycle Cost Analysis, Budget Optimisation, and Analytics.
            </div>
            <div style={{ marginTop: 10, display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {['ISO 55001 Aligned', 'HDM-4 Powered', 'GIS Integrated', 'ML-Enhanced', 'AfDB / World Bank Compliant'].map(tag => (
                <Chip key={tag} label={tag} color={C.cyan} />
              ))}
            </div>
          </div>
        </div>
...
const UNRA_LAYERS = [
  ...
  },
  {
    ...
    tools: [
      'Road Network GeoJSON (1,017 links, 21,302 km)',
