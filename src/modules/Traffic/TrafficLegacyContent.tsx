...
function LinkClassTable({ features, surfMap: _surfMap }: { features: PredFeature[]; surfMap: Record<string, string> }) {
  ...
  const VCOLS = [
    ...
    { label: 'Light Trucks',        short: 'L.Trk', color: '#66d4cf', idx: 6, key: 'vc6' as const },
                    ...
                    {VC_CLASSES.map(vc => (
                      <div key={vc.label} style={{ display:'flex', alignItems:'center', gap:8 }}>
                        ...
                        <div style={{ flex:1, height:6, background:'rgba(255,255,255,0.06)', borderRadius:3 }}>
                          <div style={{ height:'100%', width:`${(vc.pct*100).toFixed(0)}%`, borderRadius:3, background:'#66d4cf', opacity:.8 }}/>
                        </div>
                        <span style={{ fontSize:9, color:'#66d4cf', fontWeight:700, width:36, textAlign:'right' }}>{(vc.pct*100).toFixed(1)}%</span>
