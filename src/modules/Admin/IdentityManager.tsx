...
export default function IdentityManager() {
  ...
  const columns: STColumn<DirEntry>[] = [
    ...
    { key: 'status', label: 'Status', render: r => {
        ...
        return <span style={{ padding: '2px 9px', borderRadius: 999, fontSize: 9.5, fontWeight: 800, background: `${m.color}1a`, border: `1px solid ${m.color}55`, color: m.color }}>{m.label}</span>;
      } },
    { key: 'requestedAt', label: 'Requested', date: true, render: r => (
        r.requestedAt
          ? <span style={{ color: 'rgba(148,163,184,0.6)', fontSize: 11 }}>{r.requestedAt.replace('T', ' ').slice(0, 16)}</span>
          : <span style={{ color: 'rgba(148,163,184,0.4)' }}>No request date</span>
      ) },
    { key: 'decidedBy', label: 'Decided By', render: r => (
        r.decidedBy
          ? <span style={{ color: 'rgba(148,163,184,0.6)', fontSize: 11 }}>{r.decidedBy}</span>
          : <span style={{ color: 'rgba(148,163,184,0.4)' }}>Not yet decided</span>
      ) },
