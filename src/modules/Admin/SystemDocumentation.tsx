...
const DOCS: DocSection[] = [
  {
    ...
    body: [
      ...
      { bullets: [
        ...
        'Canonical data store: a managed Google Drive repository for the platform\'s source data and scripts',
  ...
  },
  {
    ...
    body: [
      { table: { head: ['Store', 'Location', 'Role'], rows: [
        ['Managed Drive repository', 'Google Drive, restricted access', 'Canonical - all data, manuals, schema, scripts'],
        ['App data bundle', 'public/data/ (44+ JSON/GeoJSON files)', 'What the deployed site reads'],
        ['Field captures', 'captures/<table>.jsonl (Drive repository root)', 'Append-only submissions from the data-entry server'],
        ['Audit logs', 'logs/audit_YYYY-MM.jsonl (Drive repository root)', 'Logins, failed logins, page views, changes'],
        ['Supabase mirror', 'Managed Supabase project (42 tables)', 'Optional mirror; RLS read-only for the public'],
        ['Git history', 'Private GitHub repository (main + gh-pages)', 'Versioned source + deploys'],
      ]}},
      { p: 'Key bundle files: network2026.geojson (road network), network_links.json (1,014 links, FY25-26), network_stats.json, link_condition_lookup.json, bridges2026.geojson (546, BMS inventory + element conditions + predicted 2026 AADT), tcs_stations.json, traffic/ROMDAS/overloading summaries, bundle.json (RoadAtlas), plus rail/ferry/airport/hydrology layers.' },
    ],
  },
  {
  ...
  },
  {
    ...
    body: [
      { bullets: [
        'Source of truth: the managed Drive repository clone (git repo, branch main)',
        'Builds run on a local build disk because node_modules cannot live on Google Drive',
  ...
  },
  {
    ...
    body: [
      ...
      { p: 'Network reference: 21,302 km (NDPIV FY25-26), 1,014 links, ~30% paved, 546 bridges, 6 maintenance regions.' },
    ],
  },
  {
    ...
    body: [
      { table: { head: ['Section', 'Contents'], rows: [
        ...
        ['Global Case Studies', '209-country road-agency literature matrix'],
