...
// ââ Road line symbology â sourced from shared/mapSymbols.ts ââââââââââââââââââ
...
// ââ Infrastructure overlay data âââââââââââââââââââââââââââââââââââââââââââââââ
interface InfraPoint { id: string; name: string; lat: number; lng: number; [k: string]: unknown }

const FERRIES: InfraPoint[] = [
  { id:'F01', name:'Bukakata Ferry',    lat: 0.1494, lng:31.9039, route:'Bukakata â Bussi Island',  capacity:'2 trucks', lake:'L. Victoria' },
  { id:'F02', name:'Nakiwogo Ferry',    lat: 0.0875, lng:32.4789, route:'Nakiwogo â Lutoboka',       capacity:'2 trucks', lake:'L. Victoria' },
  { id:'F03', name:'Wanseko Ferry',     lat: 2.1892, lng:31.3922, route:'Wanseko â Panyimur',        capacity:'1 truck',  lake:'L. Albert'   },
  { id:'F04', name:'Laropi Ferry',      lat: 3.5228, lng:31.8736, route:'Laropi (Nile crossing)',    capacity:'2 trucks', lake:'Nile'         },
  { id:'F05', name:'Obongi Ferry',      lat: 3.5128, lng:31.5736, route:'Obongi â Wadelai',          capacity:'1 truck',  lake:'Nile'         },
  { id:'F06', name:'Pakwach Ferry',     lat: 2.4589, lng:31.4939, route:'Pakwach (Nile)',            capacity:'2 trucks', lake:'Nile'         },
  { id:'F07', name:'Kaiso-Tonya Ferry', lat: 1.5003, lng:30.7428, route:'Kaiso â Tonya (L.Albert)',  capacity:'1 truck',  lake:'L. Albert'   },
];
...
// ââ Icon helpers (infra) ââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
function makeInfraIcon(type: 'ferry'|'weighbridge'|'airport'|'port', sz: number): L.DivIcon {
  ...
  const cfgs: Record<string,{g:[string,string];s:string;l:string}> = {
    ferry:       { g:['#60a5fa','#1e3a8a'], s:'#bfdbfe', l:'â' },
    weighbridge: { g:['#fcd34d','#92400e'], s:'#fef3c7', l:'â' },
    airport:     { g:['#c084fc','#4c1d95'], s:'#ede9fe', l:'â' },
    port:        { g:['#34d399','#064e3b'], s:'#d1fae5', l:'âµ' },
  };
...
// ââ Structure icon helpers (bridges & culverts) âââââââââââââââââââââââââââââââ
...
// ââ Zoom tracker (inside MapContainer) âââââââââââââââââââââââââââââââââââââââ
...
// ââ Timeline config ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
const ANIM_MIN = 1960;
const ANIM_MAX = 2026;
const ANIM_STEP = 1;
...
export default function RoadNetworkView() {
  ...
  // ââ Network stats at current anim year ââââââââââââââââââââââââââââââââââââââââ
  ...
  // ââ Current-mode stats ââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
  ...
  // ââ Style each feature âââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
  ...
  // ââ Shimmer layer â only renders on paved roads in surface/history modes ââââââ
    ...
    layer.bindTooltip(`
      <div style="font:600 11px/1.7 'Inter',system-ui,sans-serif;color:#111827;min-width:150px">
        ...
        <table style="font-size:10px;border-collapse:collapse;width:100%">
          <tr><td style="color:#6b7280;padding-right:10px;font-weight:500">Class</td><td style="color:#1d4ed8;font-weight:800">${p.road_class ?? 'â'}</td></tr>
          <tr><td style="color:#6b7280;font-weight:500">Surface</td><td style="color:${p.surface==='Bituminous'?'#1d4ed8':'#b45309'};font-weight:700">${p.surface ?? 'â'}</td></tr>
          <tr><td style="color:#6b7280;font-weight:500">Region</td><td style="color:#111827;font-weight:700">${p.region ?? 'â'}</td></tr>
  ...
  return (
    ...
    <div style={{ flex:1, display:'flex', overflow:'hidden', position:'relative' }}>
      ...
      <div style={{ flex:1, position:'relative', minWidth:0 }}>
        {loading && (
          <div style={{ position:'absolute', inset:0, zIndex:2000, display:'flex',
            ...
            <div style={{ textAlign:'center', color:'#00f5ff' }}>
              ...
              <div style={{ fontSize:13, fontWeight:700 }}>Loading road networkâ¦</div>
              <div style={{ fontSize:10, color:'rgba(0,245,255,0.5)', marginTop:4 }}>1,017 links mapped Â· {networkSummary?.official_total_km?.toLocaleString() || '21,302'} km official</div>
            </div>
          </div>
        )}
        ...
        <MapContainer center={[1.4, 32.3]} zoom={7}
          ...
          {geoData && (
            <>
              ...
              />
              {/* Shimmer highlight â paved roads only, rendered above base */}
              <GeoJSON
          ...
          {showFerries && FERRIES.map(f => (
            <Marker key={f.id} position={[f.lat, f.lng]} icon={makeInfraIcon('ferry', getInfraIconSize(mapZoom))}>
              <LeafletTooltip direction="top" offset={[0,-10]} opacity={1}>
                ...
                <div style={{fontSize:10,fontWeight:700,color:'#1d4ed8'}}>â Ferry Crossing</div>
                <div style={{fontSize:10,fontWeight:600,color:'#374151',marginTop:2}}>
                  Route: {f.route as string}<br/>Capacity: {f.capacity as string} Â· {f.lake as string}
                </div>
              </LeafletTooltip>
            </Marker>
          ))}
          {/* ââ Weighbridges ââ */}
          {showWeighbridges && WEIGHBRIDGES.map(w => (
            <Marker key={w.id} position={[w.lat, w.lng]} icon={makeInfraIcon('weighbridge', getInfraIconSize(mapZoom))}>
              <LeafletTooltip direction="top" offset={[0,-10]} opacity={1}>
                ...
                <div style={{fontSize:10,fontWeight:700,color:'#d97706'}}>â Weighbridge Station</div>
                <div style={{fontSize:10,fontWeight:600,color:'#374151',marginTop:2}}>
                  Road: {w.road as string} Â· Cap: {w.capacity as string} Â· {w.status as string}
                </div>
              </LeafletTooltip>
            </Marker>
          ))}
          ...
          {showAirports && AIRPORTS.map(a => (
            <Marker key={a.id} position={[a.lat, a.lng]} icon={makeInfraIcon('airport', getInfraIconSize(mapZoom))}>
              <LeafletTooltip direction="top" offset={[0,-10]} opacity={1}>
                ...
                <div style={{fontSize:10,fontWeight:700,color:'#7c3aed'}}>â {a.type as string} Airport</div>
                <div style={{fontSize:10,fontWeight:600,color:'#374151',marginTop:2}}>
                  IATA: {(a.iata as string)||'â'} Â· Runway: {a.runway as string}
                </div>
              </LeafletTooltip>
            </Marker>
          ))}
          ...
          {showPorts && PORTS.map(p => (
            <Marker key={p.id} position={[p.lat, p.lng]} icon={makeInfraIcon('port', getInfraIconSize(mapZoom))}>
              <LeafletTooltip direction="top" offset={[0,-10]} opacity={1}>
                ...
                <div style={{fontSize:10,fontWeight:700,color:'#0891b2'}}>âµ {p.type as string}</div>
                <div style={{fontSize:10,fontWeight:600,color:'#374151',marginTop:2}}>
                  {p.lake as string} Â· {p.operator as string}
                </div>
              </LeafletTooltip>
            </Marker>
          ))}
          {/* ââ Bridges & Culverts ââ */}
              ...
              <Marker key={s.id} position={[s.lat, s.lng]} icon={icon}
                ...
                <LeafletTooltip direction="top" offset={[0, -8]} opacity={1}>
                  ...
                  <div style={{fontSize:10,fontWeight:700,color: isBridge ? '#0891b2' : '#b45309'}}>
                    {isBridge ? 'ð Bridge' : 'ðµ Major Culvert'} Â· {s.road}
                  </div>
                  <div style={{fontSize:10,fontWeight:600,color:'#374151',marginTop:2}}>
                    {s.spanLength} m span Â· {s.noOfSpans} span{s.noOfSpans !== 1 ? 's' : ''} Â·{' '}
        ...
        {/* ââ Map top-left legend/controls ââ */}
        <div style={{ position:'absolute', top:12, left:12, zIndex:999, display:'flex', flexDirection:'column', gap:8 }}>
          ...
          <div style={{ ...glassStyle, padding:'10px 12px' }}>
            ...
            {(
              [
                { key:'ferry',       label:'Ferry Crossings', show:showFerries,      toggle:()=>setShowFerries(v=>!v),      color:'#60a5fa', rgb:'96,165,250',  icon:<span style={{fontSize:13}}>â</span> },
                { key:'weighbridge', label:'Weighbridges',    show:showWeighbridges, toggle:()=>setShowWeighbridges(v=>!v), color:'#fcd34d', rgb:'252,211,77',  icon:<span style={{fontSize:13}}>â</span> },
                { key:'airport',     label:'Airports',        show:showAirports,     toggle:()=>setShowAirports(v=>!v),     color:'#c084fc', rgb:'192,132,252', icon:<span style={{fontSize:13}}>â</span> },
                { key:'port',        label:'Ports',           show:showPorts,        toggle:()=>setShowPorts(v=>!v),        color:'#34d399', rgb:'52,211,153',  icon:<span style={{fontSize:13}}>âµ</span> },
        ...
        {/* ââ Animation timeline bar (bottom) ââ */}
        {animMode && (
          <div style={{
            ...
            <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:8 }}>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                ...
                <div style={{ display:'flex', gap:3, marginLeft:8 }}>
                  {[{l:'1Ã',v:800},{l:'2Ã',v:400},{l:'5Ã',v:160}].map(s => (
              ...
              {yearStats && (
                <div style={{ textAlign:'right', minWidth:100 }}>
                  ...
                  <div style={{ fontSize:9, color:'rgba(100,116,139,0.6)' }}>paved Â· {yearStats.pavPct.toFixed(1)}%</div>
                </div>
              )}
            </div>
        ...
        {/* ââ Selected feature panel (road-link or structure) ââ */}
              ...
              {selectedRaw && selected && (
                <div style={{
                  ...
                  <div style={{ fontSize:9, fontWeight:800, color:'rgba(0,245,255,0.5)',
                    ...
                    GeoJSON Attributes Â· {selected.link_id}
                  </div>
      ...
      {/* ââ COLLAPSE TOGGLE ââââââââââââââââââââââââââââââââââââââââââââââââââ */}
      ...
      {/* ââ RIGHT PANEL â Dashboard ââââââââââââââââââââââââââââââââââââââââââ */}
      {sideOpen && (
...
function NetworkStatsPanel({ stats, ndpiv, storyData, animYear, animMode, networkSummary }: {
  ...
  // Derive KPIs â reactive to animYear in history mode, static (inventory) in current mode
  const { totalKm, pavKm, unsKm, pavPct } = useMemo(() => {
    ...
    // Current mode â use authoritative inventory figures when available
  ...
  if (!stats && !ndpiv) return <div style={{ color:'rgba(100,116,139,0.5)', fontSize:12, textAlign:'center', marginTop:40 }}>Loadingâ¦</div>;

  return (
    <>
      <div style={sectionHead}>Uganda National Road Network</div>
      <div style={{ fontSize:9, color:'rgba(100,116,139,0.5)', marginBottom:14 }}>
        {animMode ? `Historical snapshot: ${animYear}` : 'Current state Â· FY2025-26'}
      </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', color: 'rgba(148,163,184,0.7)', marginBottom: 2 }}>
          <span>GeoJSON mapped:</span><span style={{ color: '#60a5fa', fontWeight: 700 }}>21,160 km (mapped) Â· 1,017 links</span>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', color: 'rgba(148,163,184,0.7)', marginBottom: 2 }}>
      ...
      {ndpiv && <div style={{ fontSize:8, color:'rgba(0,245,255,0.35)', marginBottom:8 }}>Network inventory Â· FY 2025/26</div>}
      ...
      {/* By class â use authoritative inventory when available */}
      <div style={{ marginBottom:16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={sectionHead}>By Road Class Â· km Paved / Unsealed</div>
      ...
      {/* By region â authoritative inventory figures */}
      <div style={{ marginBottom:16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={sectionHead}>By Maintenance Region Â· Paved / Total</div>
          ...
          <div key={r.region} style={{ marginBottom:8 }}>
            <div style={{ display:'flex', justifyContent:'space-between', fontSize:10, marginBottom:3 }}>
              ...
              <div style={{ textAlign:'right' }}>
                ...
                <span style={{ color:'rgba(0,245,255,0.5)', fontSize:8 }}> Â· {(r.paved_km/tot*100).toFixed(0)}%</span>
              </div>
            </div>
...
// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
// Dashboard Panel â detailed stats + charts
// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
  ...
  return (
    <>
      ...
      <div style={{ fontSize:9, color:'rgba(100,116,139,0.5)', marginBottom:12 }}>
        FY2025-26 Â· Uganda National Roads Â· DNR/MoWT
      </div>

      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:16 }}>
        <KpiCard val={(networkSummary?.official_total_km || 21302).toLocaleString()} unit="km" label="Total Network" sub="1,018 links Â· National inventory FY25-26" color="#00f5ff"/>
      ...
      <div style={{ marginBottom:16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={sectionHead}>Paved Road Growth 1960â2026</div>
      ...
      <div style={{ marginBottom:16 }}>
        ...
        {[
          ...
          { prog:'NDP IV',        target:'+2,000 km',  year:'2026', color:'#00f5ff', desc:'21 major road projects Â· dual carriageways to Gulu, Mbale' },
          { prog:'NDP III',       target:'+1,700 km',  year:'2025', color:'#00ff88', desc:'Completed Â· 5,360â6,405 km paved Â· OPRC contracts' },
          { prog:'NDP II',        target:'+1,100 km',  year:'2020', color:'#b967ff', desc:'Completed Â· Kampala-Jinja, Northern Bypass, Kabale-Katuna' },
          { prog:'RSDP',          target:'+900 km',    year:'2013', color:'#ff6b35', desc:'Road Sector Dev. Programme Â· World Bank co-financed' },
        ].map(p => (
      ...
      {/* Key milestones */}
      <div>
        <div style={sectionHead}>Paving Milestones Â· National Road Network FY25-26</div>
        {[
          { year:1986, km:1282, label:'Liberation â 1,282 km paved' },
          { year:1996, km:1871, label:'NRM growth â 1,871 km (+46%)' },
          { year:2006, km:2476, label:'Department of National Roads established â 2,476 km' },
          { year:2015, km:3927, label:'NDP II target â 3,927 km' },
          { year:2020, km:5360, label:'NDP III â 5,360 km paved' },
          { year:2025, km:6405, label:'Current â 6,405 km (30.1%)' },
        ].map(m => (
...
function PField({ l, v, c, span }: { l:string; v:string; c?:string; span?:boolean }) {
  return (
    <div style={{ gridColumn: span ? 'span 2' : undefined }}>
      ...
      <div style={{ fontSize:10, fontWeight:700, marginTop:2, color: c ?? '#e2eaf4' }}>{v || 'â'}</div>
    </div>
  );
}
